...
export default function ProductDrawer() {
  ...
  const productUrl = `/${locale}/producto/${slug}`
  ...
  return (
    <>
      ...
      <div
        aria-hidden="true"
        onClick={handleClose}
        className={`fixed inset-0 z-[71] bg-[#2a170f]/60 backdrop-blur-sm transition-opacity duration-300 ${isOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
      />
      ...
      <div
        ...
        className={`fixed inset-y-0 right-0 z-[72] flex flex-col w-full max-w-2xl bg-[#fff8f6] shadow-2xl shadow-[#2a170f]/40 transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-5 border-b border-[#c0c9bc]/20 shrink-0">
